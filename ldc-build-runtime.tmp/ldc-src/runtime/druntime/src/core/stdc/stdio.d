...
{
    pragma(msg, "Newlib file");
    alias _fpos_t = long;
    version(__CYGWIN__) {
        alias fpos_t = _fpos64_t;
    } else {
        alias fpos_t = _fpos_t;
    }
    extern(C) struct __sbuf {
	    ubyte *_base;
	    int	_size;
    };

    extern(C) struct _reent;
    extern(C) struct __sFILE_fake {
    ...
    private extern(C) extern shared __sFILE_fake __sf_fake_stdin;
    private extern(C) extern shared __sFILE_fake __sf_fake_stdout;
    private extern(C) extern shared __sFILE_fake __sf_fake_stderr;

    alias shared(__sFILE_fake) FILE;
    // needed for phobos
    alias __sFILE_fake _iobuf;
}
else version (CRuntime_Glibc)
{
    pragma(msg, "glibc struct");
...
else version (CRuntime_Newlib)
{
    ...
    ///
    shared FILE* stdin = &__sf_fake_stdin;
    ///
    shared FILE* stdout = &__sf_fake_stdout;
    ///
    shared FILE* stderr = &__sf_fake_stderr;
}
else version (CRuntime_Glibc)
{
    pragma(msg, "glibc files");
    enum
    {
        ///
        _IOFBF = 0,
